...
if __name__ == '__main__':
    client = pymongo.MongoClient('172.19.0.3',27017)
    db = client.flask
    ...
    f = open('statistcs_by_category.txt','w')
    for c in category_model.find():
        if 'name' in c.keys():
            anns = annotation_model.find({'category_id':c['_id']}).count()
            print(c['name'] + ' ' + str(anns))
            f.write(c['name'] + ' ' + str(anns) + '\n')
    f.close()
    quit()
